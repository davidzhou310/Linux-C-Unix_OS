...
void do_touch(char *file_name){
    ...
    if (offset == -1){
        int index = add_fat_entry(); //find a empty entry in fat and mark it as the end of file

        if (index == -1){
            perror("FAT is full");
        } else {
            add_root_entry(file_name, index, 0, REGULAR, RDWR);
        }
    } else {
...
int do_mv(char *src, char *dest){
    int offset = locate_file_entry(src);
    if (offset == -1){
        return -1;
    }
    if (locate_file_entry(dest) > 0){
        do_rm(dest);
    }
    int block_num = find_block_index(offset);
    int entry_index = find_entry_index(offset, block_num);
...
int do_rm(char *file_name){
    ...
    file_entry *entry = &directory[entry_index];
    entry->mtime = time(NULL);
    if (check_using_file(entry->first_block) == 0){
        entry->name[0] = '2';
    } else {
        entry->name[0] = '1';
    }
...
int do_cat_overwrite(ssize_t nread, char *inputBuffer, char *file_name){
    ...
    if (offset == -1) {
        ...
        while (n > 0){
            ...
            int fill = add_data(inputBuffer, block_offset, 0, filled, MAX_SIZE, n);
            if (fill == -1){
                return -1;
            }
            filled += fill;
...
int do_cat_append(ssize_t nread, char *inputBuffer, char *file_name){
    ...
    while (n > 0){
        int fill = add_data(inputBuffer, file_end_offset - end_point, end_point, filled, MAX_SIZE, n);
        if (fill == -1){
            return -1;
        }
        filled += fill;
