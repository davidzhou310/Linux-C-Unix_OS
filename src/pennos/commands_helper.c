...
int add_fat_entry(){
    ...
    for (int index = 1; index < max_entry; ++index){
        if (fat[index] == 0){  //if it is an empty entry, does it means it has nothing?
            fat[index] = MAX_INDEX;
            return index;
        }
    }
    return -1; //means the FAT is full and cannot add more file
}
    ...
    while (cur_block_idx != MAX_INDEX) {
        ...
        for (int i = 0; i < num_file_entry; i++){
            ...
            if (entry->name[0] == 0 || entry->name[0] == '1'){ //if empty or deleted, add directory
                ...
                pa_block_unmap(cur_pa_block, block_offset);
                return;
            } else if (entry->name[0] == '2'){
                if (!check_using_file(entry->first_block)){
                    strcpy(entry->name, file_name);
                    entry->first_block = index;
                    entry->mtime = time(NULL);
                    entry->size = size;
...
int add_data(char *inputBuffer, int block_offset, int file_from, int buffer_from, int limit, int remain){
    int r = remain;
    int block_size = get_block_size(get_config());
    int pa_offset = get_pa_offset(block_offset);
    ...
    int count = 0;
    while (file_from < block_size && count < limit && r > 0){
...
int can_execute_file(char *file_name){
    int perm = get_file_perm(file_name);
    if (perm == -1){
        return -1;
    }
    int can_execute = perm & EXON;
    if (can_execute > 0){
        return 1;
    }
    return 0;
